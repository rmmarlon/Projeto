...
*/
	#função para paginação de tabelas
	...
	function pagination($caminho,$valSelect,$qtdReg){
		global $oLC, $oLT, $pagina,$numPages, $totalRegistros;
		
		$pagina = isset($_GET["pagina"]) ? $_GET["pagina"] : 1;
		
		$parametro[1] = $qtdReg;
		$parametro[0] = ($pagina*$parametro[1]) - $parametro[1]; 
		
		$dao = new $caminho();
		$oLC = $dao->select($valSelect, $parametro);
		
		$dao = new $caminho();
		$oLT = $dao->select(0);
	
		$totalRegistros = count($oLT);
		$numPages = ceil($totalRegistros/$parametro[1]);
	}
	#criar paginas e limitar
